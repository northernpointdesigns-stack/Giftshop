...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 bg-[#161B22] border-b border-[#1E293B] text-[#E2E8F0] px-3 sm:px-6 py-2.5 shadow-md shrink-0 w-full">
      <div className="w-full max-w-[1720px] mx-auto flex items-center justify-between gap-3 min-w-0">
        ...
        <nav className="flex items-center gap-1 sm:gap-2">
          ...
          {isAdminLoggedIn && <button
            ...
            className={`hidden xl:inline-flex px-2.5 sm:px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors whitespace-nowrap shrink-0 ${
              activeTab === 'payouts'
                ? `bg-slate-800/80 ${activeTextClass} border border-slate-700/60`
                : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/40'
            }`}
          ...
          {isAdminLoggedIn && <button
            ...
            className={`hidden xl:inline-flex px-2.5 sm:px-3 py-1.5 rounded-lg text-xs font-bold transition-all whitespace-nowrap shrink-0 items-center gap-1.5 ${
              activeTab === 'admin'
          ...
          {/* Responsive Dropdown: visible below xl */}
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          <button
            ...
            className="hidden 2xl:flex items-center gap-1.5 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-2.5 py-1 rounded-lg text-xs font-mono transition-colors whitespace-nowrap hover:bg-emerald-500/20"
          ...
          >
            ...
            <span className="hidden md:inline">Dual Display</span>
          </button>
          {currentStaffName && (
            <button
              onClick={onLogout}
              className="flex items-center gap-1.5 text-[10px] text-slate-400 hover:text-rose-300 border border-slate-700 hover:border-rose-500/40 px-2 py-1.5 rounded-lg transition-colors whitespace-nowrap"
              title="Sign out this staff member"
            >
              <span className="hidden lg:inline max-w-20 truncate">{currentStaffName}</span>
