...
import { X, Database, RefreshCw, Download, Table, Code, FileCode } from 'lucide-react';
...
export const SqlInspectorModal: React.FC<SqlInspectorModalProps> = ({
  ...
  const handleExportJson = () => {
    ...
    downloadJsonBackup(
      `island_pos_sqlite_dump_${new Date().toISOString().split('T')[0]}.json`,
        ...
        <div className="pt-3 border-t border-[#1E293B] flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="bg-cyan-600 hover:bg-cyan-500 text-white px-3.5 py-2 rounded-xl text-xs font-bold transition-colors flex items-center gap-1.5 shadow-sm"
            >
            ...
            <button
              ...
              className="bg-slate-800 hover:bg-slate-700 text-slate-300 border border-[#1E293B] px-3.5 py-2 rounded-xl text-xs font-medium transition-colors flex items-center gap-1.5"
            >
