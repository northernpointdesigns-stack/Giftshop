...
import {
  ...
  ParsedCsvRow,
  SAMPLE_OCEAN_SEYCHELLES_CSV,
  SAMPLE_SOUVENIR_BOUTIQUE_CSV,
...
export const CsvImportModal: React.FC<CsvImportModalProps> = ({
  ...
  const handleLoadSample = (sampleType: 'ocean' | 'boutique') => {
    const sample = sampleType === 'ocean' ? SAMPLE_OCEAN_SEYCHELLES_CSV : SAMPLE_SOUVENIR_BOUTIQUE_CSV;
    handleLoadCsvContent(sample);
  };

  const handleDownloadSample = (sampleType: 'ocean' | 'boutique') => {
    const sample = sampleType === 'ocean' ? SAMPLE_OCEAN_SEYCHELLES_CSV : SAMPLE_SOUVENIR_BOUTIQUE_CSV;
    const filename = sampleType === 'ocean' ? 'Ocean_Seychelles_Catalog_Template.csv' : 'Souvenir_Boutique_Catalog_Template.csv';
          ...
          <div className="space-y-4 my-2 flex-1 overflow-y-auto pr-1">
            ...
            <div className="bg-[#0F1115] p-3 rounded-xl border border-[#1E293B] flex flex-wrap items-center justify-between gap-3 text-xs">
              <div className="flex items-center gap-2">
                ...
                <button
                  type="button"
                  onClick={() => handleLoadSample('ocean')}
                  className="bg-emerald-600/20 text-emerald-300 border border-emerald-500/30 px-2.5 py-1 rounded-lg hover:bg-emerald-600/30 font-medium transition-colors"
                >
                  Load Ocean Seychelles Catalog
                </button>
                <button
                  type="button"
                  onClick={() => handleLoadSample('boutique')}
                  className="bg-blue-600/20 text-blue-300 border border-blue-500/30 px-2.5 py-1 rounded-lg hover:bg-blue-600/30 font-medium transition-colors"
                >
                  Load Souvenir Boutique Catalog
                </button>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => handleDownloadSample('ocean')}
                ...
                >
                  <Download className="w-3 h-3 text-emerald-400" /> Download Ocean CSV Template
                </button>
                <button
                  type="button"
                  onClick={() => handleDownloadSample('boutique')}
                  className="bg-slate-800 text-slate-300 border border-slate-700 px-2.5 py-1 rounded-lg hover:bg-slate-700 flex items-center gap-1 text-[11px]"
                >
                  <Download className="w-3 h-3 text-blue-400" /> Download Boutique CSV Template
                </button>
              </div>
            </div>
